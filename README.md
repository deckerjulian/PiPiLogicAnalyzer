...
* **DreamSourceLab DSLogic:** the DSLogic Plus, U3Pro16 and U3Pro32 can be used as well, see
...
**DSLogic U3Pro16** and **DSLogic U3Pro32** of DreamSourceLab. The driver follows the open source
...
memory; a board with older firmware is updated by connecting it once to DSView. **It has not been
tested on hardware yet**; reports with the log of `pipilogicanalyzer --debug-driver` are welcome.
...
* **FPGA bitstream:** the FPGA design of the DSLogic is not open source, so its bitstreams
  (`DSLogicPlus.bin`, `DSLogicU3Pro16.bin`, `DSLogicU3Pro32.bin`) are not part of
...
* **Capture settings:** the rates of the device (up to 400 MHz on the Plus, 1 GHz on the U3
  models) are offered in a list that depends on the selected channels, as in DSView:
  ...
  *Buffer* keeps the capture in the memory of the analyzer (the memory is shared by the enabled
  ...
  lower rates. The input threshold (0–5 V, default 1.0 V) is set in the same dialog. A capture
  holds at most about one billion samples over all channels, the memory the application needs.
