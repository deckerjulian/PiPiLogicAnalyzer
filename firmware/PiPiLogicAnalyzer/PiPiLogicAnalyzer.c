...
#ifdef WS2812_LED
    #include "PiPiLogicAnalyzer_W2812.h"
#endif
...
void processData(uint8_t* data, uint length, bool fromWiFi)
{
    ...
    {
        //Have we overflowed the buffer? then inform to the host and restart reception.
        //(Checked before writing: the original tested an 8 bit index for 256 after writing
        //into a 128 byte buffer, so long frames overwrote the memory behind it.)
        if(bufferPos >= MESSAGE_BUFFER_SIZE)
        {
        ...
        else if(bufferPos > 2) //Try to parse the data
        {
            if(messageBuffer[bufferPos - 2] == 0xAA && messageBuffer[bufferPos - 1] == 0x55) //Do we have the stop condition?
            {
                ...
                //Size of the unescaped payload (start condition, command and stop condition excluded)
                int payloadLength = dest - 5;
                ...
                switch(messageBuffer[2]) //Check the command we received
                {
                    ...
                    case 0: //ID request
                        ...
                        {
                            sendResponse("PIPI_LOGIC_ANALYZER_"BOARD_NAME"_"FIRMWARE_VERSION"\n", fromWiFi);
                    ...
                    case 1: //Capture request
                        ...
                        memcpy(&request, &messageBuffer[3], sizeof(CAPTURE_REQUEST));
                        req = &request;
                        ...
                        if(simulated && req->loopCount == 0)
                            ...
                            if(req->triggerType == 1 || req->triggerType == 2 || req->triggerType == 5)
                            {
                                sendResponse("CAPTURE_ERROR\n", fromWiFi);
                                break;
                            }
                    ...
                    case 7: //Board self-test (no signal may be connected)
